...
### 关于「注入失败」的两种不同严格程度（值得知道）

本模组有两个混入配置，对失败的态度**刻意不同**：

| 配置 | 目标 | 失败的后果 |
|---|---|---|
| `yes_bsl.mixins.json` | Minecraft 自己的 `EntityRenderDispatcher` | **直接报错**（`required=true` + `defaultRequire=1`） |
| `yes_bsl.iris.mixins.json` | Iris 的内部类 | **静默失效**（`required=false` + `defaultRequire=0`） |

前者是刻意的：本模组的唯一功能就挂在这一处，而靶点签名在大版本内非常稳定；
一旦它对不上（例如换了 Minecraft 大版本），**明确报错**远比"悄悄什么都不做"更好排查。
后者则相反 —— Iris 的内部结构变动更频繁，且光影相关的失败不该拖垮游戏，所以做成软失败。

> 若你看到 `EntityRenderDispatcher` 相关的 Mixin 报错，说明本模组与该 Minecraft 版本不兼容，
> 请先移除它，或等待适配版本。
